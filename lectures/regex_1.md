...
- `[012]` is equal to `(0|1|2)`. `(012)` does not represent the same, as the pattern would look after exactly `"123"` in the string.
- Note: for German characters (or some other languages with Latin characters) you need to redefine the set of letters `[a-zA-Z]` to include also the extra letters: `[a-zA-ZäöüÄÖÜß]`. Don't forget that some alphabets (e.g. Greek) share similar letters with Latin characters (e.g., Greek "omikron", etc.); these characters are perceived differently in the regex system, therefore, you need to include them when dealing with multilingual documents.
