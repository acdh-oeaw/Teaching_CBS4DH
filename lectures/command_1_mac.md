...
* `ls -F` (option `-F` will add the following characters after pathnames: `/` for directories, `*` for executable files, `@` symbolic links etc.)
	* macOS applications (with `.app` extension) are actually directories ("application bundles"), so they will be displayed with a `/`
* `ls -j` (unsupported)
* `man ls`
* `ls data`

Dragging a file from your filesystem Finder window to the terminal
