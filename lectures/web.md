...
Hardware refers to the physical components of a computer or electronic device, like the keyboard, monitor, motherboard, and processor. It's what you can touch and see.

## What is software?

Software refers to the programs and instructions that tell a computer or electronic device what to do. It's the non-physical part that you can't touch but makes the hardware work.
...
You can author HTML, CSS, and JavaScript in _VSCode_.
...
Currently you will see nothing. However, we will create something similar to the following example in _VSCode_:
...
- CSS instructions consist of a _selector_ (which identifies what type of element will be styled) followed by a set of rules enclosed in curly braces. Each rule consist of a _property_ and a _value_, separated by a colon, and multiple rules are separated by semicolons. For example `css p {font-weight: bold; color: red;}` embolds all `<p>` elements (since the selector value is `p`) in the document and colors them (since the property name is `color`) red (since the property value is “red”). Selectors can be fine-tuned to target elements only in specific contexts or with specific attributes; we’ll see a bit of that below.
...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
    ...
    <link rel="stylesheet" type="text/css" href="static/css/style.css" />
    <script type="text/javascript" src="static/js/toggleColor.js">
      /**/
    </script>
  </head>
  <body>
